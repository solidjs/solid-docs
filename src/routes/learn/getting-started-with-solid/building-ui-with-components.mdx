...
While Solid components look like React components, they different in some important ways. Solid component functions _run only once_ to set up a component, and then the function is never run again. If you would normally put a calculation in the function body of a React component and expect it to be recomputed when the component re-renders, you may need to take a different approach with Solid. Don't worry&mdash;we'll cover all the best practices throughout this tutorial.
...
This snippet creates our `HelloWorld` component and inserts within the DOM node that has an `id` of `"root"`. At this point, we have built a representation of our user interface in JavaScript and Solid has added it to the DOM: everything needed for us to see our simple Hello World application on a web page!
