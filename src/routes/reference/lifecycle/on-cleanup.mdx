...
description: >-
  ...
  components unmount or tracking scopes dispose. Essential for proper cleanup in
...
:::note[Split effects and the apply phase]
`onCleanup` is not expected to be used inside the **apply phase** of split effects. If you need to perform cleanup for work done in the apply phase, return a cleanup function from the apply callback instead of calling `onCleanup`.
:::
