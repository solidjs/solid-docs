...
Turnkey handlers inject it automatically.
...
This types-only subpath declares the following modules:
...
This server-only module is the turnkey server build entry.
It creates a request event, runs configured middleware, dispatches the server-function endpoint when enabled, and returns the page response.
