...
version: '2.0'
description: >-
  Execute code under a specific owner in Solid for proper cleanup and context
...
:::note[Solid 2.0 — Detaching roots]
In Solid 2.0, roots created inside an owned scope are disposed with the parent. To create a fully detached root, pass `null` as the owner:

```ts
runWithOwner(null, () => {
  // Code here runs without any owner
  // Roots created here are fully independent
})
```
:::
