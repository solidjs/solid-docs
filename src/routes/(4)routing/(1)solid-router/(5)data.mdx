...
Move the reader inside the router's render prop or a route component.
...
Wrap it in `markSafeError` to send the message to the browser, or return the validation issues with `respond()`.
...
- Throw `markSafeError` or return `respond()` for messages the user should see.
