...
2. Upgrade `solid-js`, the renderer package, `vite-plugin-solid`, and framework adapters as one dependency set.
...
6. Replace resources, explicit transitions, mutations, and async boundaries.
...
## Convert mutations and explicit transitions
...
- Replace direct `createDynamic(source, props)` calls with `dynamic(source)` or the existing `<Dynamic component={...}>` component.
