...
const ROUTES = {
  ...
  createReaction: ["secondary-primitives/create-reaction.mdx", "Secondary Primitives"],
  createRenderEffect: ["secondary-primitives/create-render-effect.mdx", "Secondary Primitives"],
  createRoot: ["secondary-primitives/create-root.mdx", "Secondary Primitives"],
  createTrackedEffect: ["secondary-primitives/create-tracked-effect.mdx", "Secondary Primitives"],
  ...
  createProjection: ["store-utilities/create-projection.mdx", "Store Utilities"],
...
if (dryRun) {
  ...
  writeFiles(files);
  console.log(`Wrote ${files.length} reference file(s) to ${path.relative(repoRoot, outDir)}`);
...
function createProgram(root) {
  ...
  host.resolveModuleNames = (moduleNames, containingFile) =>
    moduleNames.map(moduleName => {
      ...
      return originalResolve(moduleName, containingFile, options, host).resolvedModule;
    });
...
function collectEntries(program, checker, root) {
  ...
  for (const entrypoint of ENTRYPOINTS) {
    ...
    for (const exportedSymbol of checker.getExportsOfModule(moduleSymbol)) {
      ...
      const line = primary.getSourceFile().getLineAndCharacterOfPosition(primary.getStart()).line + 1;
  ...
  return entries.sort((a, b) => a.route.path.localeCompare(b.route.path) || a.name.localeCompare(b.name));
}
...
function selectPrimaryDeclaration(declarations) {
  const withDocs = declarations.find(declaration => getDocs([declaration]).summary);
...
function getSignature(name, declarations, checker) {
  ...
  if (functionDeclarations.length) {
    const overloads = functionDeclarations.filter(declaration => !declaration.body);
    const targets = overloads.length ? overloads : functionDeclarations;
    return targets.map(declaration => cleanSignature(signatureFromNode(declaration))).join("\n");
  }

  const classDeclaration = declarations.find(ts.isClassDeclaration);
  if (classDeclaration) return cleanSignature(signatureFromNode(classDeclaration));

  const interfaceDeclaration = declarations.find(ts.isInterfaceDeclaration);
  if (interfaceDeclaration) return cleanSignature(signatureFromNode(interfaceDeclaration));
  ...
  if (variableDeclaration) {
    const type = checker.typeToString(
      checker.getTypeOfSymbolAtLocation(
        checker.getSymbolAtLocation(variableDeclaration.name) ?? checker.getTypeAtLocation(variableDeclaration).symbol,
        variableDeclaration
      ),
      variableDeclaration,
  ...
  if (enumDeclaration) return cleanSignature(signatureFromNode(enumDeclaration));
...
function getExportAliases(name, exportedSymbol, resolvedSymbol) {
  return exportedSymbol === resolvedSymbol || exportedSymbol.getName() === resolvedSymbol.getName()
    ? []
...
function renderMdx(entries, category, source) {
  ...
  const description =
    escapeMdxText(primary.docs.summary.split("\n\n")[0]) || `${primary.name} API reference.`;
  const title = entries.length === 1 ? primary.name : entries.map(entry => entry.name).join(" / ");
  const frontmatter = [
    "---",
...
function renderEntry(entry, source) {
  const blocks = [];
  blocks.push(escapeMdxText(entry.docs.summary) || `${entry.name} API reference.`);
...
function escapeMdxText(value) {
  return splitFencedCode(value)
    .map(part => (part.fence ? part.text : escapeAnglesOutsideInlineCode(part.text)))
    .join("");
}
...
function splitFencedCode(value) {
  ...
  while ((match = pattern.exec(value))) {
    if (match.index > lastIndex) parts.push({ fence: false, text: value.slice(lastIndex, match.index) });
    parts.push({ fence: true, text: match[0] });
    lastIndex = pattern.lastIndex;
  }
  if (lastIndex < value.length) parts.push({ fence: false, text: value.slice(lastIndex) });
  return parts;
}
...
function printDryRun(files, entries, sourceSha) {
  ...
  console.log(`Solid source SHA: ${sourceSha}`);
  console.log(`Entries: ${entries.length} (${mapped} mapped, ${unmapped} unmapped)`);
