# SolidJS Documentation

## Overview

Solid is a modern JavaScript framework designed to build responsive and high-performing user interfaces (UI).
It prioritizes a streamlined and predictable development experience, making it an great choice for developers of all skill levels.

## What is Solid?

As a JavaScript framework, Solid embraces reactivity and fine-grained updates.

Reactivity, in programming, refers to an applications' ability to respond to changes in data or user interactions.
Traditionally, when a change occurs, the entire web page would need to reload to display the updated information.

In contrast, a reactive system, such as Solid, updates only the necessary parts of the page.

With Solid's fine-grained reactivity, components update only when there is a change in the state they are dependent on.
This avoids unnecessary updates and optimizes performance.

## Advantages of using Solid

- **Performant**: Due to fine-grained reactivity, Solid updates only the parts of an application that need to be updated when data is modified.
  This minimizes unnecessary work and can result in faster load times and smoother performance overall.

- **Powerful**: Despite using less memory and processing power, Solid does not compromise on functionality.
  Thanks to its powerful API, it is capable of creating complex applications to give developers the flexibility over how and when updates happen.

- **Pragmatic**: Solid's pragmatism provides practical, effective solutions.
  Rather than sticking to rigid structures or methods, Solid provides the freedom to choose the strategies and practices that work best for you.
  With the option to integrate with other technologies, you are able to add Solid to existing projects or build entirely new ones.

- **Productive**: With a clear and predictable API, Solid makes developers' work productive and simpler.
  It is accessible regardless of experience level thanks to easier debugging and less boilerplate code.

In essence, Solid aims to strike a balance between speed, efficiency, power, and flexibility, all while providing a developer-friendly environment.
This combination of features makes it a great choice to build responsive and high-performing UIs.

## Quick links

- Follow our interactive [foundations tutorial](https://www.solidjs.com/tutorial/introduction_basics).
- Start your first project with one of our [templates](https://github.com/solidjs/templates).
- Visit the [Solid ecosystem](https://www.solidjs.com/ecosystem).
- Contribute to [our docs](https://github.com/solidjs/solid-docs-next/issues).

_Find our API documentation under the **Reference** tab_

Join the [Solid community on Discord](https://discord.com/invite/solidjs) to share your projects or get help from our community!
