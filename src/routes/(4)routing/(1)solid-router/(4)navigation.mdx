...
External URLs, non-HTTP schemes, downloads, `rel="external"`, and anchors with a `target` continue through browser behavior.

## Build URLs with `Router.paths`

The router instance's `paths` proxy is inferred from its literal route tree.
...
Navigation options control resolution, history replacement, scrolling, and location state.
...
Its setter merges values into the current query string and navigates without scrolling by default.
...
Anchors accept `replace`, `noscroll`, `state`, and `preload` attributes.
...
The `state` attribute contains JSON that becomes `location.state`.
...
Passing `true` to `retry` skips the leave handlers for the retried navigation.
