...
Here we're going to use [Railway](https://railway.app) to deploy our Solid.js project.

If you haven't heard of Railway, it's a platform where developers can host their web projects. For more information on Vercel and what they offer make sure to visit their [website](https://railway.app).

## First Step: Changing The Start Command

With Railway we'll need to make use of a different `start` command in our package.json. Instead of it being `vite` we'll have to make use of `npx http-server ./dist`. As you've noticed we'll have to have built the app in order to get the `dist` folder, so it would be better to make use of the unedited `dev` command. Here's a quick look at what this might look like in your package.json
...
**Step 2:** After authorizing Railway to be installed in your repositories, you will be redirected back to the Railway website where you can select the repository containing your Solid.js project. The screen should look something like this
...
Once you've selected the repository containing your Solid.js project you can click on the `Deploy Now` button or choose to add any environment variables your app might be making use of.
...
As you may notice a domain will not be automatically assigned to your project after deployment. With Railway you'll have to go to the settings and manually generate a domain. Here's what that looks like
...
And voila, your new Solid.js project should be deployed at the generated domain and ready to go.
...
**Step 3:** You can choose to link your Solid.js local project to an already existing Railway project using `railway link` or you could choose to create a new Railway project using `railway init` and answer the command prompts that appear in the terminal.
