...
order: 2
version: "2.0"
description: "Configures vite-plugin-solid turnkey client rendering or streaming SSR."
...
It is required when the plugin cannot use an authored client entry in client mode or an authored entry pair in SSR mode.
...
Specifies an authored server entry for turnkey SSR.
It must export:
...
The option and conventional server entry are ignored in turnkey client mode.
...
It receives the application as `props.children`.
...
It fronts every request dispatched by the generated handler, including pages and the server-function endpoint.
...
A returned component replaces `App` for that request.
Returning nothing keeps `App`.
The option is ignored in client mode and is rejected when SSR uses authored entries.
It does not run for server-function endpoint dispatch.
...
`virtual:env/server` contains server and client values, reads server values from `process.env` at server startup, and is rejected from client module graphs.
...
Hands turnkey SSR build wiring and HTTP serving to a host integration.
...
The option is ignored in client mode.
...
- `dist/server` is removed unless server functions require its handler.
...
`serverFunctions` composes with either turnkey mode.
