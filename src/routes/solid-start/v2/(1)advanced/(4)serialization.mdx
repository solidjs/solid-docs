...
SolidStart does not install a Temporal implementation. Both the client and server must provide a compatible global `Temporal` before a payload is serialized or deserialized. If any runtime targeted by your app does not provide it natively, install a global polyfill:
