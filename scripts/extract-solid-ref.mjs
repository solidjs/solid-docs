...
const FOLD_INTO = {
	...
	ProjectionOptions: "createProjection",
	PatchOp: "createStore",
	Refreshable: "createOptimisticStore",
	RevealOrder: "Reveal",
	RevealProps: "Reveal",
	ResolvedChildren: "children",
	ResolvedJSXElement: "children",
	MatchProps: "Switch",
	SignalOptions: "createSignal",
	StoreOptions: "createStore",
	StorePathRange: "storePath",
	StoreSetter: "createStore",
	isEqual: "createSignal",
};
...
function collectEntries(program, checker, root) {
	...
	for (const entrypoint of ENTRYPOINTS) {
		...
		for (const exportedSymbol of checker.getExportsOfModule(moduleSymbol)) {
			...
			if (disposition.type === "hidden" || disposition.type === "fold") {
				continue;
			}
...
function getDisposition(name, docs) {
	...
	if (FOLD_INTO[name]) {
		return { type: "fold", target: FOLD_INTO[name] };
	}
...
function printDryRun(files, entries, sourceSha) {
	const visibility = entries.reduce((counts, entry) => {
		counts[entry.route.visibility] = (counts[entry.route.visibility] ?? 0) + 1;
		return counts;
	}, {});
	console.log(`Solid source SHA: ${sourceSha}`);
	console.log(
		`Entries: ${entries.length} (${Object.entries(visibility)
