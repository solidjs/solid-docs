...
This function creates a new context object that can be used with [useContext](/reference/component-apis/use-context) and provides a context component for wrapping part of the component tree.
The default value is used when no context provider is found above in the hierarchy.

:::note[Solid 2.0 Change]
`Context.Provider` has been removed. The context object returned by `createContext` is now used directly as a component: `<MyContext value={...}>` instead of `<MyContext.Provider value={...}>`.
:::
...
With the context created in its own module, you can use it to instantiate the context provider.
...
A few important notes on how to pass data through the context API:

- The value passed to the context component is passed to `useContext` as is.
