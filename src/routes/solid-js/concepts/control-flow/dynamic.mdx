...
Instead of a more verbose [`<Switch>` and `<Match>`](/solid-js/concepts/control-flow/conditional-rendering) statement, `<Dynamic>` offers a more concise way to render components dynamically.

## Props

When working with these components, you can pass [props](/solid-js/concepts/components/props) to the component you are rendering by passing them to the `<Dynamic>` component.
