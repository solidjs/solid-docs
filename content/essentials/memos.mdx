...
Memos in Solid serve a dual purpose. 
They are both a *signal* and a *computation*, primarily used for memoizing derived state. 
...
This eliminates the need for repeated function calls.

Through the combination of observing changes in their dependencies (similar to an effect) and exposing a *read-only* reactive value (like a signal), memos can adapt well to changes to reduce any re-running.

## Using memos

To use memos, import the `createMemo` function from Solid:
...
In this example, `isEven` serves as the getter function. 
It evaluates whether the value of the `count` signal is even.
...
- With a memos ability to automatically track dependencies, any signal or other memo accessed within its function is tracked. 
As a result, the memo will re-evaluate automatically when these dependencies change, removing the need for a depenency array.
...
This means you should not modify other signals as it can lead to unexpected behaviors and make the code harder to maintain. 
...
When you add a side effect, you introduce complicated behaviours by complicating the reactivity chain and potentially introduce infinite update loops. 
...
// this is an example of a impure function in createMemo
...
In this example, you introduce unpredictability. 
If another part of the application reacts to the changes in the `message` signal, it could trigger further updates or side effects which complicate the flow of data and cause a possible infinite update loop. 
