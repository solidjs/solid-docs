...
By default all cached function will be revalidated wether the action does not return or return a "normal" response.
```jsx
...
By returning a response with solid router's `json` or `reload` helpers you can pass a key / keys to the revalidate prop as the second argument of solid's router json response helper.  You can either pass as string directly or use the cached functions `key` or `keyFor` props
