...
Welcome to Solid's new docs site! This is very much the beginnings of our new effort, and we're super excited for you to join us.
...
- We've been told that our docs are too academic/technical.

## How we're approaching new docs

Rather than editing the existing docs in-place, we decided to start a new docs site, where we can rethink structure, content, and voice in tandem. We also started a Docs Team and made it clear on Solid's Core Team that docs are a core concern. The new site also uses a writing tool called `mdx`, which is similar to markdown, but allows us to experiment with embedded rich components.

We wrote out [writing principles and a new structure](https://docs.google.com/document/d/1Z25C3LhJF4KGbf1YvnmQo7dOzF5KjEVQtoE0ucy3BJs/edit?usp=sharing) that follows the Diataxis model. We want our guides to include both project-based tutorials and topic-based how-to guides, and we want our reference section to not just include API reference, but concept explanations inspired by Diataxis's "Explanation" category.
...
Outside of the conference, our main point of communication is [Solid's Discord.](https://discord.com/invite/solidjs). Once you're there, check out the `#docs` channel. You can also contact `@jutanium` on Discord (or on Twitter if you haven't joined us yet!).

We're looking for anyone communication-minded. If you've never written a line of JavaScript and our interested in what we're doing, we want to teach you. The more outside and diverse perspectives we have, the better our team will be.
