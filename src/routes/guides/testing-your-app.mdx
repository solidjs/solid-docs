...
This guide should cover the basics of testing a Solid.js application to inspire confidence during the further development.
...
- `vitest` - testing framework including runner, assertion engine, mocking facilities
- `jsdom` - virtual DOM to simulate a headless browser environment running in node
- `@solidjs/testing-library` - testing library to simplify testing components, directives and primitives, with automatic cleanup
- `@testing-library/user-event` - simulating user events close to reality
...
test("increments value", async () => {
  ...
  const counter = queryByRole('button')
...
The render call from `@solidjs/testing-library` is used to render the component and suppy props and context. To interact with it, there is `@testing-library/user-event`. The `expect` function from `vitest` is extended with a `.ToHaveTextContent("content")` matcher from `@testing-library/jest-dom`. Running this test with
...
If the `location` option is used or the component is based on resources, the router will be lazy-loaded, thus the first query after rendering needs to be `find...`. If it should be tested whether something is not rendered, first find something that will be rendered and then use `query...`; for everything else use `get...`. 
...
test("receives data from context", () => {
  const { queryByText } = render(() => <DataConsumer />, { wrapper })
...
If you want to use fake timers with `vi.useFakeTimers()` in your tests, be sure to set it up:
...
The most convenient events are usually `click`, `keyboard` and `pointer` (to simulate touch events). Learn more about them on the [user-event documentation](https://testing-library.com/docs/user-event/intro).
...
If for example there is a [directive that handles the Fullscreen API](https://github.com/solidjs-community/solid-primitives/blob/main/packages/fullscreen/src/index.ts), it could be tested like this:

```ts title="fullscreen.test.ts"
...
Even though it takes much more configuration, it is still possible to test Solid.js applications in [jest](https://dev.to/lexlohr/testing-your-solidjs-code-2gfh) or [other testing libraries](https://dev.to/lexlohr/testing-solidjs-code-beyond-jest-39p).


## Integration/E2E testing

Some issues can only be found once the code is running in the environment it is supposed to run in. Since integration and end-to-end tests are agnostic to frameworks, all proven approaches will work equally for Solid.js.
