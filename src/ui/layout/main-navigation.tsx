...
function ListItemLink(props: { item: Entry }) {
	...
	const linkStyles = () =>
		location.pathname === props.item.path.replace(/\\/g, "/")
			...
			: "text-blue-450/90 before:hidden before:bg-blue-700 before:dark:bg-blue-200 hover:text-blue-700 hover:before:block dark:text-slate-300 ";
	return (
		<li class="relative">
			<A
				...
				if (Array.isArray(item.children)) {
					return (
						<li>
							...
							<ul
								role="list"
								class="ml-2 mt-2 space-y-2 border-l-2 border-blue-150 dark:border-slate-700 lg:mt-4 lg:space-y-4 lg:border-blue-150"
							>
												...
												<li>
													<Collapsible.Root defaultOpen={true}>
														...
														<Collapsible.Content class="navigation_collapsible">
															<ul
																role="list"
																class="ml-4 mt-2 space-y-2 border-l-2 border-blue-150 dark:border-slate-700 lg:mt-4 lg:space-y-4 dark:lg:border-slate-700 text-base lg:text-sm"
															>
	...
	return (
		<nav class="overflow-y-auto custom-scrollbar h-full md:h-[calc(100vh-7rem)] pb-20">
			<Tabs.Root defaultValue={isReference() ? "reference" : "learn"}>
				<Tabs.List class="sticky top-0 w-full z-10 dark:bg-slate-900 border-b-[1px] border-blue-150 mb-7 bg-blue-50">
					<Tabs.Trigger
						value="learn"
					...
					<Tabs.Indicator class="absolute bg-blue-700 dark:bg-blue-500 transition-all duration-250 h-[1.5px]" />
				</Tabs.List>
