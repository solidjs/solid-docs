...
<FrameworkAside framework="angular">
    Instead of JSX, Angular has its own "template compiler" that takes template strings
    and compiles them into JavaScript instructions. This serves a similar role to JSX, but
...
We can now, use `HelloWorld` as a JSX component inside another component by adding the line:
` import { HelloWorld } from "./HelloWorld"; `  and inserting `<HelloWorld />` below the `<h1>Welcome</h1>` header.
...
Take a moment to think about this. `MyApp` is a function built with a function from `<HelloWorld />`. 

<Aside>
  By building with components we are able to keep our codebase D.R.Y. (Don't Repeat Yourself). Providing flexibility and reusability.
</Aside>
...
<FrameworkAside framework="angular">
    Angular has a similar API to Fragments with [`ng-container`](https://angular.io/api/core/ng-container).
</FrameworkAside>
