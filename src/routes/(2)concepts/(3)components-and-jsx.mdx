...
Use a `ref` callback with `addEventListener` when you need native listener options such as capture or passive handling.
...
Use [`Repeat`](/reference/solid-js/components-jsx/repeat) when the input is a count instead of an array.
It reuses entries whose numeric indexes remain in the rendered range and supports an optional starting index.
...
function Rating(props: { value: number }) {
	return (
		<span aria-label={`${props.value} out of 5`}>
			<Repeat count={5}>{(index) => (index < props.value ? "★" : "☆")}</Repeat>
		</span>
	);
}
```
...
Import `dynamic` and `Dynamic` from `@solidjs/web`.
Use `dynamic` when a reactive source chooses the component or native element to render.
...
const [detailed] = createSignal(false);
const Result = dynamic(() => (detailed() ? Detailed : Compact));

export function Preview() {
	return <Result value="Current result" />;
}
```

The source may also resolve to a native tag name or an async component.
Use `Dynamic` when the selection belongs directly at one JSX call site.

```tsx
import { Dynamic } from "@solidjs/web";

function MessageField(props: { multiline: boolean }) {
	return (
		<Dynamic
