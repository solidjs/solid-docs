...
:::note[New in 1.0]
`Script` did not exist in `@solidjs/meta` 0.x — head scripts previously required the `useHead` escape hatch.
:::
...
```tsx
const Script: Component<JSX.ScriptHTMLAttributes<HTMLScriptElement> & { key?: string }>;
```
...
export default function Product(props: { product: () => { name: string; price: number } }) {
	return (
