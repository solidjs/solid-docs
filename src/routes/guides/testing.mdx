...
In Solid 2.0, directives are implemented as [ref directive factories](/reference/jsx-attributes/ref) — functions that return a callback receiving the DOM element.
...
test("toggles fullscreen", () => {
  ...
  const fs = vi.spyOn(targetElement, "requestFullscreen")
  const { setArg } = renderDirective(fullscreen, { initialValue: false })
  setArg(true)
...
export function fullscreen(active: () => boolean) {
  // Phase 1: Owned setup — create effect in component's reactive context
  let el: HTMLElement
  createEffect(
    () => active(),
    (isActive) => {
      if (!el) return
      const currentlyFullscreen = document.fullscreenElement === el
      if (isActive && !currentlyFullscreen) {
        el.requestFullscreen().catch(() => {})
      } else if (!isActive && currentlyFullscreen) {
        document.exitFullscreen()
      }
    }
  )
  // Phase 2: Apply — receives the DOM element
  return (element: HTMLElement) => { el = element }
}
```
