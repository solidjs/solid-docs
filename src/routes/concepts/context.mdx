...
:::note[Solid 2.0 — Context is the provider]
In Solid 2.0, `Context.Provider` is removed. Instead, **the context itself is used directly as a component** with a `value` prop. This reduces boilerplate and simplifies the API.
:::
...
To pass a value to the context, use the `value` prop on the context component itself, which can take in any value, including [signals](#updating-context-values).
Once a value is passed, it is available to all components that are descendants of the context provider.
...
const Provider = (props) => (
	<MyContext value="new value">{props.children}</MyContext>
);
```
...
export function CounterProvider(props) {
	return (
		<CounterContext value={props.count ?? 0}>
			{props.children}
		</CounterContext>
	);
}
