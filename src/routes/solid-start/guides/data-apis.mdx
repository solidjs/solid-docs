...
You can define data queries by wrapping your function with [`cache`](/solid-router/reference/data-apis/cache). `cache` accepts an async function to fetch data as the first argument, and a key to identify the query as the second.
...
Preloading leverages the dedupiung [capabilities](/solid-router/reference/data-apis/cache#cache-function-capabilities) of `cache`, ensuring that the query only runs once and the **result can be reused for 5 seconds**.
...
`cache` functions can throw or return a redirect if, for example, the user doesn't have authorization to access data.
...
The most important behavior of actions is their ability to **refetch queries automatically**. This behavior can also be customized declaratively using redirects and cache keys, or imperatively by returning new data. Actions stream back the updated data to the client in the same HTTP request.
...
```ts { 1, 9, 11 }
import { cache, action, reload } from "@solidjs/router"
import { storage } from "~/lib/db"

const getTodos = cache(async () => {...}, "todos")
...
Revalidating cache manually like this is an advanced optimization technique with a lot of trade-offs. You should rely on the default revalidation behavior most of the time.
...
```tsx {10, 16, 23-25, 27} 
import { action, cache, createAsync, useSubmissions } from "@solidjs/router"
import { For, Show } from "solid-js"
import { storage } from "~/lib/db"
