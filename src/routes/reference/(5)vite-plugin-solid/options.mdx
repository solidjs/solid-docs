...
With [`start`](/reference/vite-plugin-solid/start), it selects turnkey SSR instead of turnkey client mode.
An object value is rejected at configuration time.
...
- Turnkey client mode keeps application code non-hydratable; only its document shell receives an SSR transform.
