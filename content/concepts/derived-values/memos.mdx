...
Memos expose a *read-only* reactive value (like a [signal](/concepts/signals)) and track changes in their dependencies (similar to an [effect](/concepts/effects)).
...
While memos look similar to effects, they are different in that they *return a value*.
