...
[`mergeProps`](/solid-js/reference/reactive-utilities/merge-props) is a Solid utility function designed to merge multiple potentially reactive objects together.
...
[`splitProps`](/solid-js/reference/reactive-utilities/split-props) is a utility function designed to help split a single props object into multiple sets of props, retaining the reactivity of the individual properties.
...
For instances like these, Solid provides a [`children`](/solid-js/reference/component-apis/children) helper that ensures you always get the right child components without anything unwanted happening.
...
A common solution to this problem is to use [Context](/solid-js/concepts/context) to pass state to deeply nested components without having to pass props through each component in between.
