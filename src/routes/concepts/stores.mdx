...
Store properties can be accessed directly from the state proxy by directly referencing the targeted property:
...
These signals are created **lazily**, meaning they are only formed when accessed from within a tracking scope.
...
When there is a need for dealing with data outside of a tracking scope, the `unwrap` utility offers a way to transform a store into a standard [object](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object).
