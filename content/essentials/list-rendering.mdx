...
The sole property that `<For>` has is `each` , through which you can specify the data collection to loop over:
...
The `index` can be used for various purposes, such as controlling styling, setting unique keys, or any other case that requires the position of an item within the collection.
Additionally, [conditional rendering](/essentials/conditional-rendering) can be used to show how or whether certain elements are rendered based on conditions related to the `item` or `index`.
...
In this example, the [`classList`](/essentials/class-style-bindings#classList) directive is used to conditionally apply the `even` and `odd` classes to the `li` elements based on the current index.

Furthermore, within the `<For>` component, other functions can be used to compute values, manage side effects, or handle events.
There's also the ability to nest other looping components within the original `<For>` component, or to employ array methods to manage nested data structures or create more complex layouts:
...
Through this nested looping structure, the arrangement of `<For>` components provides a way to efficiently render hierarchical data. In this example, a list of categories each containing a list of items is organized and rendered in a clear and structured format.
...
Replacing the nested `<For>` loop with an `<Index>` will improve rendering performance, particularly when the items in each category being updated frequently.
...
This results in the content content at the specified index being updated to the `selected` class, while the rest of the list remains unchanged.
