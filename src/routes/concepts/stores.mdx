...
When you use this data within a reactive context, such as in a template, computed property, or effect, that is when the signals are created and dependencies are established.

For example, if you wanted to you wanted to print out every new user, adding the 
line below won't work because it is not within a tracked scope.
...
We could also set update the value of `userCount` whenever a new user is added.
