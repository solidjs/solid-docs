...
Pairing a route file with a directory creates nesting.
...
Uses `routePathFromFile` unless `config.toPath` is supplied.
...
An optional static segment such as `(en)` is rejected because the neutral path syntax represents optional parameters only.
...
- `.md` and `.mdx` files are pages when those extensions are scanned.
