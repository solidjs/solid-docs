...
Additionally, it makes sure the project's architecture is primed for React JSX file handling is vital.
...
In addition, there are several helpful type definitions to make it easier for specifying explicit types.
...
By providing default values when `createSignal` is callled, the need for explicit type specification is avoided and the possibility of the `| undefined` type is eliminated. 
...
For example, if the context is `Context<Data | undefined>`, then with using `useContext` a result of type `Data | undefined` will naturally be yielded. 
...
It is worth noting that if a generic component is created, the `Component` types will **not** be applicable. 
...
Within the scope of the `onMount` function, which runs after rendering, a non-`null` assertion (indicated by the exclamation mark `!`) can be used:
...
<Aside>
  **Caution**: 
  While TypeScript does catch incorrect usage of refs that occur before their JSX block definition, it currently does not identify undefined variables within certain nested functions in Solid. 
  Therefore, additional care is needed when using `ref`s in functions such as [`createMemo`](/reference/createMemo), [`createRenderEffect`](/reference/createRenderEffect), and [`createComputed`](/reference/createComputed).
</Aside>
...
// Permitted but will throw an error at runtime:
divRef.focus();
onMount(() => {
  divRef.focus(); 
});
...
In refactoring `UserPanel` to accept `undefined`, this minimizes the changes required when `user` goes from `undefined` to `User`.
...
To handle custom events in Solid, the attributes `on:___` and `oncapture:___` can be used. 
...
To include specific native events, certain events can chosen (e.g. `mousemove` and `pointermove`):
...
[`attr:___`]() directive allows custom attributes, on the other hand, is effective for handling string-based HTML attributes.
...
In Solid, custom directives can be applied using the the `use:___` attribute, which usually accept a target element and a JSX attribute value. 
...
* The directive function always receives a single accessor. For multiple arguments, the syntax `<div use:foo={[a, b]} />` can be used, and an accessor to a tuple should be accepted.
