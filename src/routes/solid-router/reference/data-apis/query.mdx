...
2. It fills a preload cache in the browser - this lasts 5 seconds. When a route is preloaded on hover or when preload is called when entering a route it will make sure to dedupe calls.
...
4. Serve as a back/forward cache for browser navigation for up to 5 minutes. Any user based navigation or link click bypasses this cache. Revalidation or new fetch updates the cache.
...
\*Since the internal cache is shared by all the functions using `query`, the string should be unique for each function passed to `query`. If the same key is used with multiple functions, one function might return the cached result of the other.
