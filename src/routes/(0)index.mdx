...
Then read [Reactivity](/concepts/reactivity) and [Components and JSX](/concepts/components-and-jsx) in that order; they teach signals, derived values, effects, and the run-once component model through a single example you can follow along with.
...
If you know what you need and want the signature, go to the [Reference](/reference/solid-js/reactivity/create-signal).
...
4. [Avoid unnecessary effects](/guides/avoid-unnecessary-effects): the habit that separates working Solid code from fragile Solid code.
5. [Async reactivity](/concepts/async-reactivity) and [Boundaries](/concepts/boundaries): loading data, showing fallbacks, and keeping the current screen visible while new data arrives.
6. [App structure](/building-apps/app-structure) and the rest of Building apps: the Vite plugin, server functions, sessions, and deployment.
7. [Routing](/routing/overview): how a router plugs in, then the guide for the router you chose.
