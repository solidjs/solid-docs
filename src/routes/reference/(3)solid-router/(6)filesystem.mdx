...
Eager component references are passed through without a `lazy` wrapper.
Lazy components are reused by source URL within one `fileRoutes` call.

:::note[Local-only source]
Eager component references are supported by router commit `bae09cc`, which is ahead of `origin/next`.
:::
