...
The 0.x `useHead(TagDescription)` shim is gone.
The re-exported core [`useHead`](/solid-meta/v1/reference/meta/use-head) takes `HeadTag` descriptors (`{ tag, props, key? }`) — a single tag, an array (a group), or a function (a reactive group):
