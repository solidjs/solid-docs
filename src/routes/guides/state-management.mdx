...
We can use this approach to simplify the `doubleCount` example above so we don't have to create a signal or effect:
...
This approach works great for a simple use case like this, but what if `doubleCount` needs to be used several times in our component? And what if the calculation was more computationally expensive? 
...
For cases like this, we can use [Memos](/concepts/derived-values/memos) to save the value of `doubleCount` (also referred to as memoized or cached value). 
This way it is only recalculated **1 time** when the value of `count` changes and can be reused throughout the component without re-evaluating.
