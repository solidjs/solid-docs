...
If a path is unknown ahead of time, it can be treated as a flexible parameter that is passed on to the component.
...
`id` can then be acessed within a route component with `useParams`.

**Note on Animation/Transitions**:
Routes that share the same path match will be treated as the same route.
If you want to force re-render you can wrap your component in a keyed `<Show>` like:
