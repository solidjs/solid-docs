...
An web page is displayed by rendering a component tree, which is a hierarchical structure of components.
...
Where frameworks such as [React](https://react.dev/) and [Angular](https://angular.io/) re-run components on every state change, a Solid component's lifecycle is tied to its initial run.
...
Since the logic in the component funciton is not continuously visited, it is important to set up the component correctly from the outset.
