...
`<Portal>` is used when an element requires rendering outside of the usual document flow. Challenges like stacking contents and z-index interfering with the desired intention or look. `<Portal>` helps with this by rendering elements at a different place in the document, so they are rendered as expected.
...
This creates a more accessible experience for users, as the content is no longer obscured.
    	...
    	<Callout>
    	  `<Portal>` will render wrapped by default in a `<div>` unless the mount point is `document.head`.

    	  This is so events propagate through the Portal according to the component hierarchy instead of the  elements hierarchy.
    	</Callout>

When portaling into a `SVG`, the `isSVG` prop must be used to avoid wrapping the children in a `<div>` and wrap in a `<g>` instead.
