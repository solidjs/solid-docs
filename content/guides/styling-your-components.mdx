...
Start by creating a CSS module file.
Typically, it is named with a `.module.css` extension, like `style.module.css`. 
However, other extensions such as `.scss` and `.sass` can be used.
...
2. **Using a single style:** If you only need one style from the module, import and apply it directly:
...
3. **Mixing with regular class names:** CSS module syntax can be combined with regular string class names using [template literals](https://developer.mozilla.org/en-US/docs/Learn/JavaScript/First_steps/Strings):
