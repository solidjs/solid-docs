...
The context object is then passed to the `Provider` component which is then used to provide a value to all components that are _descendants_ of the `Provider`.
...
Because `createContext` takes in an _optional_ default value, it is possible it can return `undefined` if the default value is not specified.
