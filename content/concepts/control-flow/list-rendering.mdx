...
The `<For>` component serves as a more conventional way to loop over arrays or objects in Solid.
This component manages items by updating or relocating them as the _underlying data_ it moves over changes.
This removes the need for a complete re-render of the page, which optimizes performance.
...
In this example, [`classList`](/concepts/components/class-style#classlist) is used to conditionally apply the `even` and `odd` classes to the `li` elements based on the current index.
			...
			<li key={categoryIndex}>
				...
				<ul>
					<For each={category.items}>
						{(item, itemIndex) => (
							<li key="itemIndex" onClick={() => toggleSelection()}>
								{item.name}
							</li>
						)}
					</For>
...
This nested looping structure allows for you to present hierarchical data by using the arrangement of the `<For>` components.
...
This component provides a way to manage the rendering of lists, ensuring fewer re-renders compared to the `<For>` component under _certain conditions_.
`<Index>` becomes beneficial when the order and length of the list remain _stable_, but the content may change frequently.
...
`<Index>` distinguishes itself from the `<For>` component, in that causes _fewer_ re-renders.
It is concerned primarily with the **index** of the elements in the array, thus allowing the _content_ at each index to change without a re-render.
...
Using `<Index>` will ensure that only the content at the _altered_ index is updated, providing a more efficient approach to managing dynamic lists.
			...
			<li>
				...
				<ul>
					<Index each={category.items}>
						{" "}
						// The inner `For` loop is replaced with `Index`
						{(item, itemIndex) => (
							<li
...
Unlike `<For>`, `<Index>` does _not_ trigger a re-render of the entire list when an item's list content changes, such as when it gets selected, provided the length of the list remains unchanged.
...
Another example of when _not_ to use `<For>` is when you are working with a list of input elements:
			...
			<Index each={inputs()} fallback={<p>Loading...</p>}>
				{(input, index) => (
					<input
						...
						onInput={
							(e) => setInputs()
							// logic to update the input values
						}
					/>
				)}
			</Index>
