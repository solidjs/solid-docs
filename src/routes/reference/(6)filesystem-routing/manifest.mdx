...
Its paths retain route groups.

`pageRoutes` contains only entries with `page: true`.
It nests entries by manifest path, removes groups from URL paths, and changes child paths to be relative to their parent.
...
A router explicitly assigned through `fileRoutes({ routers })` is serialized unchanged for that environment.
...
It declares `routes` as `FileRouteEntry[]` and `pageRoutes` as `FileRouteTreeEntry[]`.
It applies only to the default module ID.
...
It declares the configured module ID, types JavaScript and TypeScript refs with `typeof import(...)`, and reproduces the nested page tuple.
...
The file is regenerated at build start and after route changes, and it is written only when its content changes.
