...
- Serves `dist/client` as static files before the handler.
  `/assets/*` responses carry `Cache-Control: public, max-age=31536000, immutable`; other files carry `must-revalidate` and `Last-Modified`.
...
- Exports `listener`, a Node `(req, res)` request listener, and `serve()`, which starts the listener.

```js
import { listener } from "./dist/server/node.js";
```

The plugin warns and ignores `node` when the build writes no server bundle.
