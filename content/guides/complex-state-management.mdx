...
The context object is then apssed to the `Provider` component which is then used to provide a value to all compoennts that are **descendants** of the `Provider`.
...
Signals offer a way to synchornize and manage data shared across your components using context:
...
Context is intended to be used for data that is required more "globally" in a tree of components.
For example, if you have a component that passes props to one child component, you can forgo the use of context and just use props.
