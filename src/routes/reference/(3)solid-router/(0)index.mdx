...
The package currently emits several implementation exports through its declaration graph.
`RouterContext`, `_mergeSearchString`, and `createBeforeLeave` are not documented as application APIs.
Router context is explicitly marked opaque and internal in the source, `_mergeSearchString` is prefixed as internal, and `createBeforeLeave` supports `useBeforeLeave`.

Imports from unpublished internal module paths are not supported.

:::note[Source revision]
This reference covers local revision `3daa4d071df288280eb0de996039cf591d8173f8`.
Its final three commits are ahead of `origin/next`.
:::
