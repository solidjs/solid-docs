...
Control flow based narrowing refers to narrowing the type of a value through control flow. 
...
In this instance, using optional chaining is a good solution:
...
This is similar to using the keyed option, but instead provides an accessor so the children are not recreated whenever `when` changes.
...
For example, when a `UserPanel` component strictly accepts a `User`:
...
This is similar to using the keyed option, but an accessor is provided so that the children are not recreated whenever `when` changes.
...
Sometimes, this may not be possible. For instance, with a `UserPanel` component that strictly accepts a `User`:
...
Casting can also be a valid solution so long as the assumption is valid:
...
These directives typically accept two arguments: the target element, and the value passed to them by the JSX attribute.
The conventional `Directives` interface directly types the types that can be passed to the directive (i.e. the type of `value` in `<div use:foo={value} />`).
The newer `DirectiveFunctions` interface accepts the type of a function instead, from which it will derive the valid types of element(s) and values. 

There are additional considerations:
* A single accessor is always passed to the directive function, so to accept multiple arguments, use `<div use:foo={[a, b]} />`, and accept an accessor to a tuple.
* This applies even when using a boolean directive (`<div use:foo />`), or when passing a static value to a directive (`<div use:foo={false} />`).
* `DirectiveFunctions` can accept functions that do not strictly meet the type requirements, in which case they will be ignored. 
...
Although the `Directives` interface can be used to restrict the value passed to the directive as a JSX attribute, using `DirectiveFunctions` ensures both the element and value align with the expected types, as demonstrated below.
