...
The handler can report the original value and return—or resolve to—a safer replacement for the client. SolidStart awaits the handler before serializing the response, allowing a monitoring service to flush first.
