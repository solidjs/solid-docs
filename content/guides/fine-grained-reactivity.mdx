...
* [Stores](/essentials/stores) which are [proxies](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy) that create, read, and write signals under the hood.
* [Memos](/essentials/memos) resemble [effects](/essentials/effects) but are distinct in that they *return* a signal and optimize computations through caching. 
...
[Signals](/essentials/signals) are like mutable variables that can point to a value now and another in the future. 
...
[Effects](/essentials/effects) are functions that are triggered when the signals they depend on point to a different value.
