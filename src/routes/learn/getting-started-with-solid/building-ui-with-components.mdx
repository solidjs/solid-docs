...
While Solid components look like React components, they differ in some important ways. Solid component functions _run only once_ to set up a component, and then the function never runs again. If you would normally put a calculation in the function body of a React component and expect it to be recomputed when the component re-renders, you may need to take a different approach with Solid. Don't worry&mdash;we'll cover all the best practices throughout this tutorial.
