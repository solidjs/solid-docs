...
Cached function provide few useful methods for getting the key that can be used in cases with invalidation.
...
The cache can be revalidated using the `revalidate` method or the `revalidate` keys are set on the response from the actions.
