...
Outside components, [`createRoot`](/reference/solid-js/advanced/owner-introspection/create-root) creates an owner and gives you its disposer:
...
Use this in tests, in framework integrations, and for reactive state that lives outside the component tree.
...
Call it: `quantity()`.
Writing `{quantity}` on its own in JSX does work, because Solid calls a function it finds in a JSX expression, but a template string or concatenation stringifies the function first.
