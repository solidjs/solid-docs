...
This JSX represents a `<div>` element with the text `Hello World!` inside. As if we had written it in an HTML document! 
As a means of showing you the power and flexibility of Solid, let's add some inline styling to our `<div>`:
(Please note: inline style isn't recommended.)
...
<FrameworkAside framework="vue">
  Vue has its own "template language" in a designated file type called `.vue`. `.jsx` serves a similar role in Solid, but isn't directly integrated into JavaScript. We can write a JSX element anywhere we could write a JavaScript expression, and there's no enforced separation between template and data.
</FrameworkAside>

<FrameworkAside framework="svelte">
  Sveltehas its own "template language" in a designated file type called `.svelte`.`.jsx` serves a similar role in Solid, but isn't directly integrated into JavaScript. We can write a JSX element anywhere we could write a JavaScript expression, and there's no enforced separation between template and data.
</FrameworkAside>
