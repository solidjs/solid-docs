...
Fetching data from a server or other sources is a key part of most applications.
...
Learn more about automatic revaliation in the [actions documentation](/solid-router/concepts/actions#automatic-data-revalidation).
...
Anything inside the suspense boundary will be managed by Solid's streaming and concurrency system, and if it's not ready, the boundary's fallback UI will be displayed.
