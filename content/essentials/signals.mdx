...
In the top-level code, which includes the `console.log` statement, execution happens only **once** — when the component is initially rendered.
...
While the [`createMemo`](/essentials/memos) function is often used to achieve similar results, such as what is shown with `double()`, you can simply put a signal inside a function to create similar effect:
...
These functions gain reactivity from the signal they access.
They do not, however, store a value but will update any effects that depend on them and trigger an update if it is included within a component body.