...
Since `HttpStatusCode` is just a component, it can be used with [`ErrorBoundary`](/solid-js/reference/components/error-boundary), [`Show`](/solid-js/reference/components/show), [`Switch`](/solid-js/reference/components/switch-and-match) or any of the other JSX control-flow components. 
...
Errors can be thrown from inside these fetchers and caught by the nearest [`<ErrorBoundary>`](/solid-js/reference/components/error-boundary) component from where the data is accessed.
...
Note that when streaming responses [`renderStream`](/solid-js/reference/rendering/render-to-stream), the HTTP Status can only be included if added _before_ the stream first flushed.
