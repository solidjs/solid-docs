...
Using the `on:__` form, which will add an event listener to the element, commonly known as a `Native Event`.
Using the `on__` form which will add an event listener to the document and dispatch it to the element, commonly known as a `Delegated Event`.
The reason for the two forms is performance, delegated events could save some resources and perform better on  commonly used events.

To add an event handler, you can use any of the `on` or `on:` forms as a prefix followed by the event name, and assign it a function. The function will be called whenever the event is dispatched.
...
Delegated events are case-insensitive, while Native events are case-sensitive. When using delegated event handlers in Solid, they can be written using camelCase or all lowercase.
...
<Callout type="tip" title="Event handlers are not reactive.">
  ...
  Attaching and detaching listeners on elements is a costly process. To prevent performance degradation, event handlers are not reactive.
...
If necessary, you can write your handlers to call a reactive source. In the following example, handleClick represents a prop that could dynamically change its value to any function you wish.
...
Instead of attaching event listeners to every individual element, Solid uses synthetic event delegation.
This means that event listeners are attached to the document element, and events are dispatched to the relevant elements as they bubble up.
...
If you need to attach an event listener to an element that is not supported by Solid's event delegation, you can use the [on:](/reference/jsx-attributes/on-and-oncapture) form.
...
<Callout type="caution">
  ...
  1. Delegated events are not removed on tracking scope disposal. This may or may not be desired depending on the situation.
  2. `event.preventDefault()` won't work as expected as the event is attached to the `document` instead of to the `element`.  When in need to `preventDefault` use a Native Event instead.
</Callout>
...
By binding events in this way, the overhead of using JavaScript's [bind](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_objects/Function/bind) method and an aditional closure is avoided.
