...
Solid Router simplifies routing in Solid applications to help developers manage navigation and rendering. 
Solid Router supports defining routes using JSX or objects passed via props. 
This section will focus on the practical implementation of Solid Router using JSX.
...
The `A` component creates an anchor tag that creates routes within an application. 
Additionally, the component takes in an `href` prop which is the URL path that the link will navigate to.
...
Solid router offers a `Navigate` component that works similarly to `A`, however, it will *immediately* navigate to the provided path as soon as the component is rendered.
...
However, only leaf `Route` nodes, or the innermos `Route`, will be given a route. 
...
As long as there are appropriate parent component `<Outlet />` placeholders defined at each level, you can nest routes indefinitely using this approach. .

## Dynamic Routing

Dynamic routing lets an application handle URLs where some segments are not known ahead of time. 
In Solid, you can create a dynamic route by using a colon (:) before a segment in the `path` prop of a `Route` component. 
...
This primtive can be especially useful with other Solid primitives, such as [`createResource`](/reference/createResource) and [`createSignal`](/reference/createSignala).
This creates dynamic behaviours based on the route parameters.
...
In this example, every time the `id` parameter changes, the `fetchUser` function is called to fetch the new user data.
...
Routes can be specified as optional by adding `?` to the end of a parameter name. 
This allows for more flexible routing patterns where certain segments of the path may or may not be present.
...
This hook allows you to retrieve the results of the `data` function once the route is loaded.
...
The default behaviour in Solid Router is to use `location.pathname` to determine the route path.
However, there is more flexibility built into the router which gives you the option to easily switch to hash mode through the `source` property on the `<Router>` component.
...
`useMatch` creates a [Memo](/essentials/memos) that returns match information if the current path matches the provided path, which is useful for determining if a given path matches the current route.
...
<Aside>
  Access to more comprehensive insights and details on Solid Router can be accessed through its [GitHub repository](https://github.com/solidjs/solid-router).
</Aside>