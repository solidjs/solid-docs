...
<Callout>
	API routes are prioritized over page route alternatives. If you want to have
	them overlap at the same path remember to use `Accept` headers. Returning
	without a response in a `GET` route will fallback to page route handling.
</Callout>
