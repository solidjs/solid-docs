...
A write made inside an effect function cannot join the batch that is finishing; it starts the next one.
Readers of the copied signal therefore update one batch after readers of the source, and the screen paints in between.
A memo is a compute function, so it lands in the same batch as its inputs and there is no frame in which the two disagree.
:::
...
In the `Prefer` version each click runs the [`action`](/reference/solid-js/lifecycle-actions/action), which runs the request as one transaction: writes made inside it, such as an optimistic value, are held until it settles.
...
A setter called there starts a new batch, which is the copy pattern this guide opened with.
