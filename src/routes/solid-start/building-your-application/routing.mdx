...
Routing is one of the most important concepts in understanding SolidStart.
This is because everything in your application starts with a route.
This includes the compiler, the initial request, and almost every user interaction afterward.

There are two types of routes:

- **UI routes** &mdash; define the user interface in your app
- **[API routes](/solid-start/building-your-application/api-routes)** &mdash; define the serverless functions in your app._createMdxContent
...
File based routing is a way of defining your routes by creating files and folders in your project.
This includes your pages, but also your API routes.

SolidStart traverses your `routes` directory, collects all the routerSchema, and then makes them accessible using the [`<FileRoutes />`](/solid-start/reference/router-reference/file-routes).
...
When you have a path that is nested but wish for it to have a seperate Layout, you can escape the nested route by using the `.` character. 
