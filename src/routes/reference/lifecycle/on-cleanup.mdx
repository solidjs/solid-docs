...
When used in reactive contexts, such [`createEffect`](/reference/basic-reactivity/create-effect), [`createMemo`](/reference/basic-reactivity/create-memo) or a [`createRoot`](/reference/reactive-utilities/create-root), it runs when the reactive scope is diposed or refreshed.
...
const Component = () => {
	...
	return <button>Document has been clicked {count()} times</button>;
};
```
