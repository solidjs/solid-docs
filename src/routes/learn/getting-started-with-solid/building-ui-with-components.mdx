...
Components are functions that define how your application will look. Each component generally represents a small part of your application's entire user interface. Splitting your application into components has many advantages: components are small, reusable, easily-tested, and enable different team members to work on different parts of the application without stepping on each other.
...
Solid provides `render` function that will render our app inside this `div`:
...
This snippet creates our `HelloWorld` component and inserts within the DOM node that has an `id` of `"root"`. This enables us to see our simple Hello World application. The `render` function _also_ enables Solid's reactive system and will allow our application to come to life as we make in more dynamic.
