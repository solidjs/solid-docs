...
It is not necessary to add it to `vite.config`, since `vite-plugin-solid` automatically detects and loads it if present.
...
Use `getAllBy...` if there are multiple elements matching the same query, otherwise use `getBy...`.
