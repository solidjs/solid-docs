...
**Note:** We can include an `aria-label` on our Add button to make the action taken by the button clear to screen readers. As a matter of principle, Solid will never get in the way of writing accessible code.

## Finishing the app

Let's plug this resource fetching and display codee into our `AddBook` file, which will complete the application:
