...
`<Portal>` helps with this by rendering elements at a different place in the document, bringing an element to a different place within the document flow so they can render as expected.
...
This creates a more accessible experience for users, as the content is no longer obscured.
    	...
    	<Callout>
    	  `<Portal>` will render wrapped by default in a `<div>` unless specifically directed to `document.head`.
