...
`solid-js` also exports an [`action`](/reference/solid-js/lifecycle-actions/action); that one runs a generator as a reactive transaction and is covered in [Async reactivity](/concepts/async-reactivity#mutations-from-a-client-only-list-to-a-server-backed-one).
