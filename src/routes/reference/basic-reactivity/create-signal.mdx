...
:::note
	If you want to store a function in a Signal you must use the function form for the setter:

	```tsx
	setValue(() => myFunction);
	```

	In Solid 2.0, passing a function as the first argument to `createSignal` creates a **writable derived signal** (see [Function form](#function-form-writable-derived-signal) below). To store a function as a plain value, wrap it in an arrow:
...
| `pureWrite` | `boolean`                                 | `false` | If true, suppresses the dev warning when writing to this signal inside a reactive scope. See [`pureWrite`](#purewrite) below.                                                                                                                                      |
