...
```tsx
const [data] = createSignal(query, searchBooks);
```
...
<Show when={!data.loading} fallback={<>Searching...</>}>
  <ul>
    <For each={data()}>
      {(book) => (
        <li>
          {books.title} by {book.author} <button>Add</button>
        </li>
      )}
    </For>
  </ul>
...
Finally, we'll want to make sure we the `<button />` in each list item adds the item to our book list.
...
**Note:** We can include an `aria-label` on our Add button to make the action taken by the button clear to screen readers. As a matter of course, Solid will never get in the way of writing accessible code.

## Finishing the app

<Tabs files={["App.tsx", "BookList.tsx", "AddBook.tsx", "searchBooks.ts"]} selected="AddBook.tsx">
  <Tab name="App.tsx">
