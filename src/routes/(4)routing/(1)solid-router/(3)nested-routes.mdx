...
export default function RequireSignIn(props: RouteProps) {
	...
	return (
		<Show
			when={session().user}
			fallback={<a href="/sign-in">Sign in to continue</a>}
		>
			{props.children}
		</Show>
	);
...
Each component then reads its own query through a memo, as on the [Data](/routing/solid-router/data) page, and the `Loading` boundary above them decides what to show while they are pending.
...
- [Navigation](/routing/solid-router/navigation): typed `paths`, active and pending link styling, and navigation guards for sections such as `RequireSignIn`.
