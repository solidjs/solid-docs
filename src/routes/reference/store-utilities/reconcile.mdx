...
const unsubscribe = store.subscribe(({ todos }) => {
  setState((s) => { s.todos = reconcile(todos)(s.todos) });
});
onCleanup(() => unsubscribe());
```

:::note
In Solid 2.0, for derived stores created with the function form of `createStore`, consider using [`createProjection`](/reference/store-utilities/create-projection) which handles reconciliation automatically for derived store data.
:::
