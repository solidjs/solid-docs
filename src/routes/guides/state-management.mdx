...
:::note[Solid 2.0 — Batched updates]
Signal updates are batched by default via microtask scheduling. After calling a setter, the value is visible immediately through the getter, but DOM updates are deferred until the next microtask. If you need to force synchronous DOM updates, use `flush()`.
:::
...
However, there may be times when you want to perform additional side effects when the state changes — such as logging, syncing to an external system, or updating the DOM.

[Effects](/concepts/effects) are reactive primitives designed for side effects. In Solid 2.0, effects use a split model: a **compute** function that tracks reactive dependencies, and an **apply** function that performs the side effect:
...
function Counter() {
	...
	createEffect(
		() => count(), // compute: track count
		(value) => {
			console.log("Count changed to:", value); // apply: side effect
		}
	);
...
Writing to signals inside effects, memos, or the component body now warns in dev. Effects are for side effects (DOM updates, logging, external syncing) — not for deriving state. If you need a value based on other state, use a [derived signal](/concepts/derived-values/derived-signals) or [memo](/concepts/derived-values/memos) instead.
:::

For example, if you want to display a doubled value of `count`, **don't** use an effect to write to another signal. Instead, derive the value:

```jsx
// ❌ Anti-pattern: writing signals in an effect
createEffect(
	() => count(),
	(value) => setDoubleCount(value * 2) // warns in dev!
);

// ✅ Use a derived signal
...
To avoid the concept of prop drilling, which is the process of passing props through multiple components, Solid offers [stores](/concepts/stores/basics) to manage state in a more scalable and maintainable manner.
