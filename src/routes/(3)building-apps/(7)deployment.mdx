...
- It serves `dist/client` first.
  Hashed `/assets/*` files get `Cache-Control: public, max-age=31536000, immutable`; other files get `must-revalidate` and a `Last-Modified` header.
  A path containing `..` cannot leave the directory.
...
`dist/server/node.js` exports `listener`, a `(req, res)` function that does everything the emitted server does, and `serve()`, which starts the listener on `PORT` and `HOST`.
...
Pass `listener` to `http.createServer`, or mount it as the last handler of a framework that accepts a Node request listener.
Static files and the handler come together; to serve `dist/client` yourself or to skip it, import `handleRequest` from `dist/server/server.js` and write the bridge.

A bridge you write must meet the same requirements the emitted entry does.
It must stream the request body for methods other than `GET` and `HEAD`, forward multiple `Set-Cookie` headers as separate values, and pass the Node request as `event.nativeEvent` if middleware or server functions read it.
