...
Upon initialization, an effect will run *once*, regardless of whether it has any dependencies.
This is useful for setting up the effect and initializing variables or subscribing to [signals](/concepts/signals).
After this run, the effect will only be triggered when any of its *dependencies* change.
...
An inner effect will *not* affect the outer effect.
Signals that are accessed within an inner effect, will *not* be registered as dependencies for the outer effect.
When the signal located within the inner effect changes, it will trigger only the *inner effect* to re-run, not the outer one.
