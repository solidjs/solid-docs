...
If the parent needs to be its own route, it can be specified separately:
...
Use nesting through the use of `props.children` passed to the route component:
...
Nesting can be done indefinitely.
It is important to note, however, that only leaf nodes will become their own routes.
