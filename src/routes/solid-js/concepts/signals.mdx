...
Signals are the primary means of [managing state](/solid-js/concepts/intro-to-reactivity#state-management) in your Solid application.
They provide a way to store and update values, and are the foundation of [reactivity](/solid-js/concepts/intro-to-reactivity) in Solid.
...
You can create a signal by calling the [`createSignal`](/solid-js/reference/basic-reactivity/create-signal) function, which is imported from `solid-js`.
...
When a signal is called within a [tracking scope](/solid-js/concepts/intro-to-reactivity#tracking-changes), the signal adds the dependency to a list of subscribers.
...
```
    	...
    	<Callout>
    	  A tracking scope can be created by [`createEffect`](/solid-js/reference/basic-reactivity/create-effect)  or [`createMemo`](/solid-js/reference/basic-reactivity/create-memo), which are other Solid primitives.
...
To learn more about how to use Signals in your application, visit our [state management guide](/solid-js/guides/state-management).
