...
* Memos resemble effects but are distinct in that they return a signal and optimize computations through caching. 
They update based on the behavior of effects, ideal for computational optimization.
...
Signals are like mutable variables, that can point to one value now and another in the future. 
...
The effect takes a function that is re-invoked whenever any of the signals it relies on changes, such as `count`.
...
Here, the `createEffect` function initiates a `setTimeout`, which delays the console log. 
