...
Each of those has a shape in other frameworks that involves a loading flag, a request counter, or an `AbortController`, and each has a shorter shape in Solid because a computation that returns a promise is a value.
...
One line in the handler keeps the graph as it is:
