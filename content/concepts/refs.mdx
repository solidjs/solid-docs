...
Since elements in Solid can be added or removed from the DOM based on state, however, you need to wait until the element is attached to the DOM before accessing it.
...
While this works, this component is now only able to be used once in the DOM.
If you were to use this component multiple times, you would have multiple elements with the same `id`, which is invalid HTML.
...
It lets you access the element directly without having to wait until it is attached to the DOM, and it can be used multiple times without having to worry about duplicate `id`.
