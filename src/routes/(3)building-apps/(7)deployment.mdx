...
Without `ssr: true`, start mode prerenders the static document shell to `dist/client/index.html` and removes `dist/server` when the application has no server functions.
...
Without `ssr: true` and without server functions, start mode removes `dist/server` after prerendering the shell; there is nothing to run, so deploy `dist/client` as static files.
