...
async function attachUser(_request: Request, next: () => Promise<Response>) {
...
`getRequestEvent()` returns the same event the page render and the server functions for this request see, so `locals.userId` set here is readable from a server function without passing it around.
...
Order matters in the usual way: put the session reader before anything that needs `locals.userId`, and put the API handler after it if API routes need the user.
