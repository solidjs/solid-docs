...
Solid 2 stages reactive work on its queue so consecutive writes can share one update pass.
The click changes the signal immediately, but the DOM update can remain queued when `fireEvent.click` returns.
Call [`flush()`](/reference/solid-js/reactivity/flush) after a synchronous interaction to drain pending work before reading the DOM.
...
Replace jsdom with the browser dependencies:

```sh
pnpm add -D @vitest/browser-playwright@^4.0.0 playwright@^1.56.0
