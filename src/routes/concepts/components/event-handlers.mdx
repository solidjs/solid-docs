...
: [`on__`](/reference/jsx-attributes/on_): adds an event listener to the `document` and dispatches it to the `element`. This can be referred to as a as a _delegated event_.
...
In other words, events do not dynamically adjust, and the bindings are not reactive.
...
In the following example, `handleClick` represents a prop that 
has the flexibility to adopt any function.
...
This keeps event listeners attached to the `document` element, and dispatched to the relevant elements as the event bubbles up.
...
Some things to keep in mind include:
 - Delegated event listeners remain active even if the element that added them is removed. 
For example, if a `div` listens for `mousemove` and is later removed, the `mousemove` events will still be dispatched to the `document`. 
Delegated event listeners are added _once_ per event type and handle all events of that type. Once setup, the delegated event listener will call the handlers attached to any existing elements, but the listener itself will persist even if all elements with that handler type are removed.
...
Since these events happen in specific circumstances, they do not benefit from the performance improvements of event delegation.
...
- `event.stopPropogation()` does not work as expected since events are attached to the `document` rather than the `element`.
