...
If you only want to avoid passing "some" props through a "few" layers, adjusting your component hierarchy may be an easier solution, when applicable.
[Signals](/concepts/signals), when injected directly into a component that needs them, will always be the simplest and most explicit solution.

Context, on the other hand, is designed to share data that can be considered public or "global", i.e. information that is regularly accessed by many components in your application's component tree.
It offers a way to access state across the hierarchy without passing props through intermediate layers and injecting them directly into components.
