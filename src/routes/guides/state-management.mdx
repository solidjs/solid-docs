...
Here is a [Solid Playground example](https://playground.solidjs.com/anonymous/b05dddaa-e62a-4c56-b745-5704f3a40194) so to see the example in action.

## Derived state

When you want to calculate new state values based on existing state values, you can use derived state.
This is a useful pattern when you want to display a transformation of a state value to the user, 
but do not want to modify the original state value or create a new state value.
...
We can use this approach to simplify the `doubleCount` example above so we don't have to create a 
signal or effect
...
This approach works great for a simple use case like this, but what if `doubleCount` needs to be
used several times in our component? And what if the calculation was more computationally expensive? 

A derived signal could lead to performance issues as it would be re-evaluated for each use of `doubleCount`.
...
For cases like this, we can use [Memos](/concepts/derived-values/memos) to save the value of `doubleCount` 
(also referred to as memoized or cached value). This way it is only recalculated **1 time** when 
the value of `count` changes and can be reused throughout the component without re-evaluating.

In Solid we have the [`createMemo`](/reference/basic-reactivity/create-memo) function:
...
The `doubleCountMemo` function is only called once even though it is accessed the same number of times
as `doubleCount`.

Here is a [Solid Playground example](https://playground.solidjs.com/anonymous/288736aa-d5ba-45f7-a01f-1ac3dcb1b479) similar to the above example.
