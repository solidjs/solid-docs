...
This page continues the store from the [Solid Router introduction](/routing/solid-router): a product page and a cart.
...
When the server function throws, production builds replace the message with a generic one unless the error is marked safe; [Mutations and responses](/building-apps/server-functions/mutations-and-responses) covers `markSafeError` and returning validation issues with `respond()`.
