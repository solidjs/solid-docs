...
Here is a [Solid Playground example](https://playground.solidjs.com/anonymous/b05dddaa-e62a-4c56-b745-5704f3a40194) to see the example in action.
...
This approach can be used to simplify the `doubleCount` example above.
The additional signal and the effect can be replaced with a derived signal:
...
This approach works great for a simple use case like this, but what if `doubleCount` is used several times in our component? 
And what if the calculation is more computationally expensive? 

A derived signal could lead to performance issues. 
It would be re-evaluated not just for each time `count` is changed, but also for each use of `doubleCount`.
...
For cases like this, we can use [Memos](/concepts/derived-values/memos) to store the value of `doubleCount` (also referred to as a memoized or cached value). 
This way it is only recalculated **1 time** when the value of `count` changes and can be accessed many times throughout the component without re-evaluating for each additional use.

In Solid we have the [`createMemo`](/reference/basic-reactivity/create-memo) function to create a memo of a function:
...
The `doubleCountMemo` function is only called once even though it is accessed the same number of times as `doubleCount`.
