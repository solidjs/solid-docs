...
Effects are meant primarily for side effects that read but don't write to the reactive system: it's best to avoid setting signals in effects, which without care can cause additional rendering or even infinite effect loops. Instead, prefer using [createMemo](/solid-js/reference/basic-reactivity/create-memo) to compute new values that depend on other reactive values, so the reactive system knows what depends on what, and can optimize accordingly.

The first execution of the effect function is not immediate; it's scheduled to run after the current rendering phase (e.g., after calling the function passed to [render](/solid-js/reference/rendering/render), [createRoot](/solid-js/reference/reactive-utilities/create-root), or [runWithOwner](/solid-js/reference/reactive-utilities/run-with-owner)).
...
In particular, [refs](/solid-js/reference/jsx-attributes/ref) will already be set.
...
If you'd rather an effect run immediately even for its first run, use [createRenderEffect](/solid-js/reference/secondary-primitives/create-render-effect) or [createComputed](/solid-js/reference/secondary-primitives/create-computed).

You can clean up your side effects in between executions of the effect function by calling [onCleanup](/solid-js/reference/lifecycle/on-cleanup) inside the effect function.
