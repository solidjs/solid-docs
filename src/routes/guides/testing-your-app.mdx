...
### Adding testing packages to your project
...
Additionally, in your `package.json` add a `test` script calling `vitest`:
...
<Callout title="Using TypeScript">
...
The prefix (get/query/find) and middle part (By/AllBy) depend on if the query should wait for the element to (not) appear, if it should throw if the element cannot be found, and how it should handle multiple matches:
...
They get a reference to the HTML element and the property from the directive prop as an accessor as arguments.
