...
By doing so, they offer an efficient way to use these derived values, cache the results of expensive or frequently accessed computations, and minimize unnecessary work. 
...
- Unlike a [basic derived signal](/essentials/signals#derived-signals) or [effects](/essentials/effects), memos are optimized to execute only _once_ for each change in their dependencies.
...
For example, with `setCount(0)`, `isEven()` updates to `true`; however, `setCount(2)` might recompute `isEven()` and will yield `true`, prompting no further updates in components relying on `isEven()`.
...
However, to minimize unnecessary updates that occur as a result of a change, it is better practice to convert any unnecessary effects into memos.
This can help to minimize unnecessary updates in other parts of the application.
And, generally speaing, opting for memos is the recommended approach over the use of effects:
...
**Note**: In situations where side effects, such as DOM updates, are important, using the `createEffect` function should be used over a memo.