...
Solid Router allows you to configure your routes using JSX. This section will demonstrate how to set up routes
 ...
 component which acts as a page layout.
...
Add `active` and `inactive` here and remove from section below.
</div>
...
This way, the `<Users>` and `<Home>` components will only be loaded if you're navigating 
to `/users` or `/`, respectively.
...
Routes that share the same path match will be treated as the same route. If you want to force 
...
Even with smart caches, it is possible that we have waterfalls both with view logic and with 
lazy loaded code. With load functions, we can instead start fetching the data parallel to loading 
the route, so we can use the data as soon as possible. The load function is called when the Route 
is loaded or eagerly when links are hovered.
...
A common pattern is to export the load function and data wrappers that correspond to a route in a 
dedicated `route.data.js` file. This way, the data function can be imported without loading anything else.
...
The return value of the `load` function is passed to the page component when called at any time other 
than `"preload"`, so you can initialize things in there, or alternatively use our new Data APIs:
...
To prevent duplicate fetching and to trigger handle refetching we provide a cache API 
that takes a function and returns the same function.
