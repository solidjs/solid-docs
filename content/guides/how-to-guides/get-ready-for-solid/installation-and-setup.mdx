...
> cd my-app
> npm install # yarn add or pnpm install
# To run the server
> npm run dev
```
...
In the command below you can replace `tailwindcss` with `windicss`, `sass` or `unocss` to get a template that already has one of these already configured.
...
## Setup Typescript In Solid.js Javascript Projects
...
Notice how we made use of the environment variable in our Solid.js component, instead of using `process.env` that a lot of developers are familiar with, we're making use of `import.meta.env` which is specific to Vite.
...
<Aside>For more information on environment variables in Vite and how to make use of intellisense on your environment variables in Typescript, check out the <Link href="https://vitejs.dev/guide/env-and-mode.html#env-files" target="_blank">Vite Documentation</Link></Aside>