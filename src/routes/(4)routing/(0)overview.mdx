...
Shared layouts belong inside it, in the router's root render prop or a layout route, so they can read the location:
...
// Prefer: the header is in the root render prop, where router primitives work
<Router>
	{(props) => (
		<>
			<Header />
...
- Put shared layout inside the router's render prop, not around the router, so it can use router primitives.
