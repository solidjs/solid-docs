...
Keep the selection in a store keyed by id, so each row reads only its own key:
...
<li class={{ selected: selected[todo.id] }} onClick={() => select(todo.id)}>
	...
</li>;
```
...
Either form replaces the Solid 1 `createSelector`.
