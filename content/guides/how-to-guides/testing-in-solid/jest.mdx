...
At the time of writing, Jest only officially supports the <a class="footnote" id="cjs-module-format" href="#fn-cjs-module-format" target="_self">CJS module format</a> directly and uses a transformation pipeline to run other formats and transpiled JS. Since Solid.js uses transpiled code, we need to set it up.
...
<IfTS>
<li id="fn-babel">[Babel](https://babeljs.io/) is a versatile utility to convert one kind of JS code into another one. It is currently the de-facto standard to convert Solid.js JSX into JS. <a href="#babel">back</a></li>
</IfTS>
...
<li id="fn-primitives">In many frameworks, hooks/primitives are named `use[Something]`. Solid.js uses `create` as a prefix for its reactive primitives to denote that these are only called once inside the component. <a href="#primitives">back</a></li>
</ol>
</section>
