...
This guide is about picking one.
The mechanics live in [App structure](/building-apps/app-structure) and [Rendering and SSR](/concepts/rendering-and-ssr).
...
Server functions blur this line.
With `serverFunctions: true` in client mode, the pages stay static but the endpoint that serves the function calls is still a server handler in `dist/server`, so the host has to run something after all.
If you want a static deployment and server-side data, prerendering is the mode that gives you both.
...
Move it behind an effect or `onSettled`, guard it with `isServer`, or load the component with `clientOnly`.
The initial output must also match between server and client, or hydration cannot claim the server-rendered nodes.
[Server and client boundaries](/concepts/rendering-and-ssr#server-and-client-boundaries) lists the tools.
