...
source_repo: "solidjs/templates"
source_ref: "main"
source_sha: "cbbd8ba26ed7f909455abe142de74e204b039974"
source_path: "solid-v2"
---
...
When `test.environment` is unset, `vite-plugin-solid` supplies a jsdom environment and Vitest resolves that dependency before it starts the browser pool.
...
Vitest runs the session test outside the turnkey server, so the template aliases that virtual module to this test stub:
...
The proxy reads `process.env` when code accesses a property.
...
It does not run the application environment schema or the turnkey server boot process.
