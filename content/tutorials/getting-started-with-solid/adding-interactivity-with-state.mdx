...
In Angular, this might just be a class property that you mutate. This is one of the biggest differences that Solid has from Angular.
...
{/* TODO: Add mention of Angular's `trackBy` and add `<Aside>` advanced */}
...
<FrameworkAside framework="vue">
  This is equivalent to `const totalBooks = computed(() => books.length)` in
  Vue.
</FrameworkAside>
