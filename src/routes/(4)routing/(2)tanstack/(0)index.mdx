...
The `fullstack-tanstack` project shape is the `fullstack` shape with the client half swapped out.
...
Three things to notice, because each is a deliberate departure from a default.

**The loader is a hint, not a gate.**
...
The `GET` wrapper matters here more than in the Solid Router template.
Without it every server-function call is a `POST`, and while the single-flight consumer is mounted, every `POST` asks the server to re-run the page's loaders and hold the response until they settle.
...
- [Middleware and API routes](/building-apps/middleware-and-api-routes): the `src/api` handlers.
- [Deployment](/building-apps/deployment): same `dist/client` and `dist/server` contract as the Solid Router shape.
