...
Since initialization is a **one-time event**, if a signal is accessed *outside of a tracking scope*, it will not be tracked.
...
function Counter() {
	...
	return (
		<div>
			<span>Count: {count()}</span> {/* ✅ will update when `count()` changes. */}
			<button type="button" onClick={increment}>
				Increment
			</button>
		</div>
