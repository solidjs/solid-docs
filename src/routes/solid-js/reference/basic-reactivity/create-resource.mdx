...
(Note: if you anticipate errors, you may want to wrap `createResource` in an [ErrorBoundary](/solid-js/reference/components/error-boundary).)

As of **v1.4.0**, `data.latest` will return the last returned value and won't trigger [Suspense](/solid-js/reference/components/suspense) and [transitions](#TODO); if no value has been returned yet, `data.latest` acts the same as `data()`. 
