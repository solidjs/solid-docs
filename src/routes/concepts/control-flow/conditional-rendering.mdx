...
### Function children with accessors

When using function children with `<Show>`, the child function receives a **narrowed accessor** for the truthy value. Call the accessor to get the value:
...
The `keyed` prop tells `<Show>` to treat the value identity as the switching condition. When the identity of the value changes, the children are recreated:
