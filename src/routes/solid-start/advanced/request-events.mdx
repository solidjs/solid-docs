...
In SolidStart, request event are retrieved using the [`getRequestEvent`](/reference/server-utilities/get-request-event) from `@solidjs/web` anywhere on the server.
