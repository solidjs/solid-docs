...
A `with` method can be used when typed data is required. This removes the need to use `FormData` or other additional hidden fields. The `with` method works similar to [`bind`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_objects/Function/bind), which applies the arguments in order.
