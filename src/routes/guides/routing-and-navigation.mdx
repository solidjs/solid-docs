...
This section will demonstrate how to set up routes and navigate to them using the [`<Router>`](/reference/solid-router/components/router), [`<Route>`](/reference/solid-router/components/route) and [`<A>`](/reference/solid-router/components/a) components. Additionally, you will add a root component that acts as a page layout.
...
This will always be there and won't update on page change.
It is the ideal place to put top-level navigation and Context Providers.
...
Add each route to a `Router` using the `Route` component.
Specify a path and a component to render when the user navigates to that path.
