...
The `useAction` primitive takes an action as its parameter and returns a function that, when called, triggers the action with the provided parameters.
...
After changing data on the server, the data used in the application can become stale.
To solve this, Solid Router automatically revalidates all [queries](/solid-router/concepts/actions) used in the same page after a successful action.
This ensurs any component using that data is automatically updated with the freshest information.
