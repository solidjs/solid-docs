...
You can then access that `id` from within a route component with [`useParams`](/reference/solid-router/primitives/use-params).
...
This primitive can be especially useful with other Solid primitives, such as [`createResource`](/reference/basic-reactivity/create-resource) and [`createSignal`](/reference/basic-reactivity/create-signal), which can create dynamic behaviors based on the route parameters.
...
Here, the `matchFilters` prop has been added which provides a way to validate the `parent`, `id` and `withHtmlExtension` parameters against the filters defined in `filters`.
...
For SSR, the static router can be used directly. Alternatively, the browser router will default to it on the server by passing in the URL.
