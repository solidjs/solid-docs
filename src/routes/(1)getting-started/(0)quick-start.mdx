...
- `<Loading>` is a boundary.
  When a page inside it is waiting on data, the boundary shows the fallback instead of the page, and it keeps the current page visible when you navigate to one that is still loading.
