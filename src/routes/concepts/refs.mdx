...
This can be done by using [`onSettled`](/reference/lifecycle/on-settled) to wait until the element is attached to the DOM before accessing it:
...
The `use:` directive namespace is removed in Solid 2.0. Instead, directives are expressed as **`ref` directive factories** — functions that return a callback to receive the element.
:::
