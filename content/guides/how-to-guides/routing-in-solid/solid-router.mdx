...
You can simply switch to hash mode by using `<HashRouter>` component instead of `<Router>` component.
...
You can simply switch to memory mode by using `<MemoryRouter>` component instead of `<Router>` component.
And use `createMemoryHistory` for navigation.
