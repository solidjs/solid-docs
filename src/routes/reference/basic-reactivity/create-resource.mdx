...
2. When server rendering resources especially when fetching when embedding Solid in other systems that fetch data before rendering, you might want to initiate the resource with this prefetched value instead of fetching again and having the resource serialize it in it's own state.
