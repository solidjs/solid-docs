...
In JSX based frameworks such as Solid, Preact, React and the likes, components are defined using a function which returns a JSX element. Here's an example of a simple component:
...
As you can see in the above code there's a `console.log(count())` statement. This will only be run once, and will not be re-run when the application state changes.
