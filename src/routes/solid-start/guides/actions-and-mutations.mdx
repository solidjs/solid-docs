...
1. Create an action and assign it a unique name.
...
When invoked in a form, the action automatically receives the [`FormData`](https://developer.mozilla.org/en-US/docs/Web/API/FormData/FormData) object, allowing you to extract field data using native `FormData` methods.
...
To display a pending UI while the action is executing:
...
3. Use the `pending` property returned by `useSubmission` to display the pending state.
...
2. Call `useSubmission` with your action.
3. Use the `error` property returned by `useSubmission` to display the error message.
...
4. Use the `result` property returned by `useSubmission` to display the validation errors.
...
To safely interact with your database or ORM in an action, ensure it is server-only by adding [`"use server"`](/solid-start/reference/server/use-server) as the first line of your action:
...
If you prefer not to use a <form> element, you can programmatically invoke an action using the `useAction` primitive:
...
To redirect users after an action completes, use the [`redirect`](/solid-router/reference/response-helpers/redirect) helper:
