...
[`event delegation peculiarities`](#event-delegation-peculiarities) for
...
### Event delegation peculiarities
...
Event delegation it's also designed for event propagation through the JSX tree,
...
1.  Delegated event listeners are added to the `document` once per event type.
    ...
    present in the `document`. Considering the following example, a `div`
...
This can be simply solved by switching the `button` event to use a Native Event
as follows:
