...
A route definition connects a path pattern to a component, optional preload work, nested routes, search validation, and metadata.
...
Its `const` type parameter preserves path literals that TypeScript would otherwise widen, so `createRouter({ routes })` can infer typed paths from the array.
...
Use `:name` for a required segment, `:name?` for an optional segment, and `*name` for the remaining path.
...
`matchFilters` rejects parameter values that do not satisfy an array, regular expression, or predicate.
...
It also changes the corresponding `Router.paths` argument to `number`.
...
Its resolution is cached and added to the compiled route tree.
...
An optional named `route` export supplies `preload`, `matchFilters`, `search`, and `info`.
...
Eager component passthrough comes from router commit `bae09cc`, which is ahead of `origin/next`.
:::
