...
For example, if you wanted to update `store.userCount` with every new user, simply writing `setStore("userCount", store.userCount.length)` would not work.
...
[View on Eraser![](https://app.eraser.io/workspace/maDvFw5OryuPJOwSLyK9/preview?elements=2Q6tB00RWK_vx-wSh3-fVw&type=embed)](https://app.eraser.io/workspace/maDvFw5OryuPJOwSLyK9?elements=2Q6tB00RWK_vx-wSh3-fVw)
...
`reconcile` will determine the differences between new and existing data, initiating updates only when there are *changed* values which avoids unneccessary updates.
