...
The primitive we want to test is `onClickOutside`, which calls a function given in its argument, and we want to test it by clicking both inside and outside and check whether our function had been called:
...
<li id="fn-primitives">In many frameworks, hooks/primitives are named `use[Something]`. Solid.js uses `create` as a prefix for its reactive primitives instead to denote that these are only called once inside the component. <a href="#primitives">back</a></li>
</ol>
</section>
