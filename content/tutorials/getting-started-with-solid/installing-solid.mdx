...
import { IfConfig } from "~/components/configurable/IfConfig";
...
The <IfConfig check="codeFormat" forValue="jsx" fallback={<code>vite.config.ts</code>}>`vite.config.js`</IfConfig> file configures the Vite project.
- Everything we'll be doing in this tutorial takes place in the `src` folder.
- The starting point for our site is `index.html`. This provides a basic HTML skeleton, and links to <IfConfig check="codeFormat" forValue="jsx" fallback={<code>src/index.tsx</code>}>`src/index.jsx`</IfConfig>.
- <IfConfig check="codeFormat" forValue="jsx" fallback={<code>src/index.tsx</code>}>`src/index.jsx`</IfConfig> is the starting point for our Solid app. 
This in turn imports <IfConfig check="codeFormat" forValue="jsx" fallback={<code>src/App.tsx</code>}>`src/App.jsx`</IfConfig>, which is our first component. We'll discuss components next!

</div>

  <IfConfig check="codeFormat" forValue="jsx" fallback={<img
