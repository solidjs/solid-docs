...
Options:
  ...
  --out <path>         Output directory. Defaults to src/routes/v2/reference.
	--overrides <path>   Sidecar MDX directory for persistent human-authored additions.
											 Defaults to src/routes/v2/reference-overrides.
...
function renderEntry(entry) {
	...
	if (ENTRY_CALLOUTS[entry.name]) {
		sections.push(["callout", undefined, ENTRY_CALLOUTS[entry.name]]);
	}
	...
	if (entry.docs.deprecated) {
		sections.push([
			"deprecated",
			undefined,
			`> Deprecated: ${escapeMdxText(entry.docs.deprecated)}`,
		]);
	}
	...
	if (entry.docs.remarks) {
		sections.push(["remarks", "## Remarks", escapeMdxText(entry.docs.remarks)]);
	}
	...
	if (related.length) {
		sections.push(["related", "## Related", related.join("\n")]);
	}
			...
			const content =
				heading === undefined
					...
					: body === undefined
						? heading
						: `${heading}\n\n${body}`;
			return genBlock(`${slug}:${key}`, content);
		})
		.join("\n\n");
}
...
function writeFiles(files) {
	...
	for (const file of files) {
		...
		const existing = fs.existsSync(file.fullPath)
			? fs.readFileSync(file.fullPath, "utf8")
			: "";
		fs.writeFileSync(
			file.fullPath,
			mergeManagedContent(existing, file.content)
		);
	}
}

function mergeManagedContent(existing, generated) {
	if (!hasHumanContent(existing)) return generated;
	let merged = existing;
	const pattern = /\{\/\* @gen:([^ ]+) \*\/\}[\s\S]*?\{\/\* \/@gen:\1 \*\/\}/g;
	let match;
	while ((match = pattern.exec(generated))) {
		const block = match[0];
		const start = `{/* @gen:${match[1]} */}`;
		const end = `{/* /@gen:${match[1]} */}`;
		if (merged.includes(start) && merged.includes(end)) {
			merged = replaceRegion(
				merged,
				start,
				end,
				block.slice(start.length, -end.length)
			);
		}
	}
	return merged;
...
function hasHumanContent(content) {
	if (!content) return false;
	const stripped = content
		.replace(/^---[\s\S]*?---/, "")
		.replace(/\{\/\*[\s\S]*?\*\/\}/g, "")
		.replace(/\{\/\* @gen:([^ ]+) \*\/\}[\s\S]*?\{\/\* \/@gen:\1 \*\/\}/g, "");
	return stripped.trim().length > 0;
}

function replaceRegion(content, start, end, body) {
	const from = content.indexOf(start);
	const to = content.indexOf(end);
	if (from === -1 || to === -1 || to < from) return content;
	return content.slice(0, from + start.length) + body + content.slice(to);
}
...
function cleanGeneratedFiles(dir) {
	...
	for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
		...
		if (hasHumanContent(content)) continue;
...
function toReferenceHref(routePath) {
	...
	return `/v2/reference/${normalized}`;
}
