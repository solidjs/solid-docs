...
function Counter() {
  ...
  /* When count() is called, the count" signal is tracked as a dependency of this effect,
  ...
  return (
    /*
      ...
      registers count as a dependency when count() is called
    */
...
This destructuring assignment is only code that actually accesses `props.value`!
