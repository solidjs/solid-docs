...
The <IfTS fallback={<code>vite.config.js</code>}>`vite.config.ts`</IfTS> file configures the Vite project.
- Everything we'll be doing in this tutorial takes place in the `src` folder.
- The starting point for our site is `index.html`. This provides a basic HTML skeleton, and links to <IfTS fallback={<code>src/index.jsx</code>}>`src/index.tsx`</IfTS>.
- <IfTS fallback={<code>src/index.jsx</code>}>`src/index.tsx`</IfTS> is the starting point for our Solid app. 
This in turn imports <IfTS fallback={<code>src/App.jsx</code>}>`src/App.tsx`</IfTS>, which is our first component. We'll discuss components next!
