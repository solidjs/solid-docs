...
Components act as the basic units for Solid applications.
These code snippets create reusability and, when combined, facilitate the development of applications.

Solid helps you to write reactive HTML through [JSX](/concepts/understanding-jsx).
...
However, as applications grow, their component trees can become intricate.
...
The nesting capabilities of components highlight their power, as they offer a way to break down applications into smaller, more manageable pieces.
...
This includes creating the `count` signal and the `console.log(count())` statement.
Once these lines are run, they will not be executed again for the life of this component instance, meaning the log statement will show only _once_ in the console.
...
Solid allows for conditional rendering within components, which allows for different parts of a component to be displayed based on state or other criteria.
...
<Aside>
	To simplify the Solid component, you can reformat the example above with new
	components for each side of the conditional.
</Aside>
...
Furthermore, Solid provides built-in [control-flow](/concepts/control-flow/conditional-rendering) components like [`<Show>`](/reference/show), which you can employ for a more simple conditional rendering experience:
...
To be reusable across different parts of an application or project, components need to be exported from one module and imported into another.
