...
Components can communicate with their child components by passing properties called `props`. To demonstrate the power of `props`, let's customize the bookshelf by adding your name! In the example below, we update the app to display "Solid's Bookshelf," but you should use your name.
