...
`useParams` can be especially useful with other Solid primitives, such as [`createMemo`](/reference/basic-reactivity/create-memo) and [`createSignal`](/reference/basic-reactivity/create-signal), which can create dynamic behaviors based on the route parameters.
...
const User = () => {
	...
	const data = createMemo(() => fetchUser(params.id)); // Async memo suspends until resolved
