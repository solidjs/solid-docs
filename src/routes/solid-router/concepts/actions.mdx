...
To invoke an action, use [`useAction`](/solid-router/reference/data-apis/use-action) in a component.
This returns a callable function that executes the action, passing any arguments if the function wrapped by `action` expects them.
