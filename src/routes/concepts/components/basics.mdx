...
This includes setting up [signals](/concepts/signals), [stores](/concepts/stores/basics), [effects](/concepts/effects), and other reactive elements.
...
In Solid 2.0, reading signals or reactive props **at the top level of a component body** will produce a dev warning. This includes destructuring props. Always read reactive values inside JSX, `createMemo`, `createEffect`, or wrap in `untrack` for intentional one-time reads.

```tsx
// ❌ warns: destructuring reactive props
function Bad({ name }) {
	return <div>{name}</div>;
}

// ✅ access props inside JSX
function Good(props) {
