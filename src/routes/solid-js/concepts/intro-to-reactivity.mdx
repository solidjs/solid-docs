...
Reactive primitives, such as [effects](/solid-js/concepts/effects), can be used to create subscribers.
...
The UI of a Solid application is built using [JSX](/solid-js/concepts/understanding-jsx).
...
To learn more about managing state in Solid, visit the [guide on state management](/solid-js/guides/state-management).
...
[`batch`](/solid-js/reference/reactive-utilities/batch) can be used to delay an update so the subscriber runs after each signal has been updated.
...
If you want to dive deeper, visit the [guide on fine-grained reactivity](/solid-js/advanced-concepts/fine-grained-reactivity).
