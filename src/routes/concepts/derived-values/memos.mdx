...
In Solid 2.0, `createMemo` can return a **Promise** or **AsyncIterable**. When a memo returns a Promise, any consumer that reads it while unresolved will **suspend** — wrap reads in [`<Loading>`](/reference/components/loading) to handle the loading state. This replaces `createResource` from Solid 1.x.
