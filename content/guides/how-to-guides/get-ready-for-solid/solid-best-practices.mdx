...
function Parent() {
  ...
  return (
    <>
      ...
      <MyComponent value={value}/> // 👈 the removal of the parenthesis 
    </>
  )
}

interface Props {
  value: Accessor<boolean> // 👈 the use of Accessor<boolean> instead of boolean
}
...
Solid offers a number of in-built components that make it easy to implement lists and conditional rendering in JSX. It's recommended to use these component APIs instead of arbitrary javascript in your JSX. This ensures that your code stays properly reactive and doesn't behave unexpectedly or produce unwanted outcomes.
