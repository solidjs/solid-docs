...
<Callout>
	A mutable state can be passed around and mutated anywhere, which can
...
This helper function simplifies making multiple changes to a mutable Store (as returned by [createMutable](#createMutable)) in a single [batch](/reference/reactive-utilities/batch), so that dependant computations update just once instead of once per update. The first argument is the mutable Store to modify, and the second argument is a Store modifier such as those returned by [reconcile]('#reconcile') or [produce](#produce). (If you pass in your own modifier function, beware that its argument is an unwrapped version of the Store.)
