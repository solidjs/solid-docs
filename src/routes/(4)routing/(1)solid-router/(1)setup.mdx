...
The returned value is both the provider component and the application-wide static routing instance.
Its `routes`, `config`, `paths`, and `match()` members do not represent one visitor's current location.
...
It remains mounted while matched route components render through `props.children`.
...
Browser history is the default on the client.
The current request URL is the default location on the server.
...
A nested router is not supported.
Compose one route tree instead, using extracted route arrays or [lazy route subtrees](/routing/solid-router/route-definitions#load-a-route-subtree-lazily).
