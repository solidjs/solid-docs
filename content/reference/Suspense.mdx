...
<Aside type="note">
  Suspense is triggered by reading a resource inside the {"<Suspense>"} boundary.
  Components wrapped with suspense still run fully, just as they would without suspense.
  However, code wrapped in `onMount` and `createEffect` only run after the resource resolves.
</Aside>
