...
A hold that was acknowledged but still ran long is not a diagnostic either; it is recorded as `late` in the feedback tables so the cost is visible without blaming code that waited correctly.
...
- For a value that updates too often, name the scopes, enable attribution, reproduce, and ask `DEV.attribution.why(scope)`.
